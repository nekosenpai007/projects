...
def is_nimda_worm(packet):
    if packet.haslayer(TCPConnector) and packet[TCP].dport == 80:
...
def packet_callback(packet):
    ...
    if src_ip in blacklist_ips:
        ...
        log_event(f"Bloacking blacklisted IP: {src_ip}")
        return
    
    if is_nimda_worm(packet):
        print(f"Blocking Nimda source IP: {src_ip}")
        os.system(f"iptables -A IMPUT -s {src_ip} -j DROP")
...
print("Monitering network traffic....")
